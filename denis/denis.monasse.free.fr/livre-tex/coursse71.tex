...
Remarque~12.5.2 Si la base \mathcal{E} est orthonormée, alors \Omega = I\_n et
\mathrmMat~
(u^∗,\mathcal{E}) = ^t\
...
Démonstration Soit \mathcal{E} une base de E, \Omega =\
\mathrmMat (\phi,\mathcal{E}) et A =\
\mathrmMat (u,\mathcal{E}). Alors \psi\_u(x,y) =
...
u)^2, soit
\mathrm{det}~ u = ±1.
L'application O\_\Phi(E) \rightarrow~\-
1,1\,
u\mapsto~\mathrm{det}~
...
u(x) = \\sum ~
x\_iu(e\_i) et comme u(\mathcal{E}) est orthonormée, \Phi(u(x))
= \\sum ~
...
A)^2, soit
\mathrm{det}~ A = ±1.
L'application O\_K(n) \rightarrow~\-
1,1\,
A\mapsto~\mathrm{det}~
