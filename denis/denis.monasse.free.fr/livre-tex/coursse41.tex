...
séries) Soit (x\_n,q)\_(n,q)\in\mathbb{N}~\times\mathbb{N}~ une famille de nombres
...
\item
  ...
  \mathbb{N}~,\textbar{}x\_n,q\textbar{}\leq \alpha~\_n
\item
...
(\\sum ~
\_n=0^+\infty~x\_n,q\right )\_q\in\mathbb{N}~
...
tel que \\sum ~
\_n=M+1^+\infty~\alpha~\_n \textless{} \epsilon\over
4. On a alors
...
Remarque~7.7.1 Le lecteur qui a dé\jmathà des connaissances sur les séries de
...
Théorème~7.7.2 Soit u = (u\_n,p)\_(n,p)\in\mathbb{N}~\times\mathbb{N}~ une famille
...
\\sum ~
\_nu\_n,p est absolument convergente pour tout p \in \mathbb{N}~ et
...
(\textbar{}u\_n,p\textbar{})\_(n,p)\in\mathbb{N}~\times\mathbb{N}~, on constate que
la série \\sum ~
\_p\left
(\\sum ~
