...
\subsection{2.6.1 Généralités}

Définition~2.6.1 M_K(m,n) =
\(a_i,j)_ 1\leqi\leqm \atop
1\leqj\leqn \ est un K-espace vectoriel de dimension mn.
Il admet pour base la famille (E_k,l)_ 1\leqk\leqm
\atop 1\leql\leqn  avec

 E_k,l = (\delta_i,j^k,l)_ 1\leqi\leqm
...
(a_i,j)_ 1\leqi\leqm \atop 1\leqj\leqn 
construite de fa\ccon équivalente par (i)
\forall~j \in [1,n], u(e_j~)
= \\sum ~
_i=1^ma_i,jf_i (ii)
\forall~i \in [1,m], \\forall~~j \in
[1,n], a_i,j = f_i^∗(u(e_j))
=\langle
f_i^∗∣u(e_j)\rangle
...
Produit de matrices A = (a_i,j) \in M_K(m,n) et B =
(b_i,j) \in M_K(n,p). On définit AB = (c_i,j) \in
M_K(m,p) par
